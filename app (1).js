...
(function () {
  ...
  function findingCard(f, idx) {
    ...
    return (
      ...
      (f.estrategiaDefensa
        ? '<div class="mt-2 bg-amber-50 border border-amber-100 rounded p-2">' +
          '<p class="text-xs text-amber-900"><span class="font-semibold">Estrategia de defensa: </span>' +
          escapeHtml(f.estrategiaDefensa) + "</p></div>"
        : "") +
      (f.weightBoosted
        ? '<p class="mt-2 text-[10px] font-bold uppercase tracking-wider text-rose-700">⚠ Severidad elevada por modo Cero Tolerancia</p>'
        : "") +
      "</div></div></div>"
    );
  }

  function dimensionGroupHtml(group) {
    const tone = group.tone || "slate";
    return (
      '<div class="mb-5">' +
      '<div class="flex items-center justify-between gap-3 mb-3 px-1">' +
      '<h4 class="text-sm font-bold text-' + tone + '-700 flex items-center gap-2">' +
      '<span class="w-1.5 h-5 rounded bg-' + tone + '-500"></span>' +
      escapeHtml(group.label) +
      "</h4>" +
      '<span class="text-[10px] font-bold uppercase tracking-wider bg-' + tone + '-100 text-' + tone + '-700 px-2 py-0.5 rounded-full">' +
      group.findings.length + " hallazgo" + (group.findings.length === 1 ? "" : "s") +
      " · peso " + group.totalWeight + "</span>" +
      "</div>" +
      '<div class="space-y-3">' +
      group.findings.map((f, i) => findingCard(f, i)).join("") +
      "</div></div>"
    );
  }

  function capacidadResidualHtml(result) {
    const profile = currentProfile && currentProfile();
    if (!profile) return "";
    const opp = {
      objeto: result.meta.objeto, valor: 0, modalidad: result.meta.modalidad,
    };
    // Cuantía tomada del proceso si está disponible en el formulario o del último seguimiento
    const valorEl = $("#f-cuantia");
    if (valorEl) opp.valor = Number(valorEl.value) || 0;
    const r = LICITA.analyzer.analizarCapacidadResidual(profile, opp);
    if (!r || !r.aplica) return "";
    const tone = r.stop ? "rose" : r.ratio >= 1.3 ? "emerald" : "amber";
    const verdict = r.stop ? "⛔ INSUFICIENTE — Revisar antes de ofertar" :
                    r.ratio >= 1.3 ? "✓ Cubre cómodamente" : "⚠ Justo en el límite";
    return (
      '<div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 animate-fade-in">' +
      '<div class="flex items-center justify-between gap-3 flex-wrap mb-3">' +
      '<h3 class="text-base font-semibold text-slate-900">📐 Capacidad Residual (K) · Decreto 1082/2015</h3>' +
      '<span class="text-[10px] font-bold uppercase tracking-wider bg-' + tone + '-100 text-' + tone + '-700 px-2 py-0.5 rounded-full">' + verdict + "</span>" +
      "</div>" +
      '<div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-3">' +
      '<div class="bg-slate-50 rounded-lg p-3"><p class="text-[10px] text-slate-500 uppercase font-bold">K práctica</p><p class="text-base font-bold text-' + tone + '-700">' + formatCOP(Math.round(r.kPractica)) + "</p></div>" +
      '<div class="bg-slate-50 rounded-lg p-3"><p class="text-[10px] text-slate-500 uppercase font-bold">Presupuesto</p><p class="text-base font-bold text-slate-800">' + formatCOP(r.presupuesto) + "</p></div>" +
      '<div class="bg-slate-50 rounded-lg p-3"><p class="text-[10px] text-slate-500 uppercase font-bold">Contratos obra</p><p class="text-base font-bold text-slate-800">' + r.factores.contratosObraRecientes + "</p></div>" +
      '<div class="bg-slate-50 rounded-lg p-3"><p class="text-[10px] text-slate-500 uppercase font-bold">Cobertura</p><p class="text-base font-bold text-' + tone + '-700">' + Math.round(r.ratio * 100) + "%</p></div>" +
      "</div>" +
      '<p class="text-xs text-slate-600 mt-3 leading-relaxed">' + escapeHtml(r.recomendacion) + "</p>" +
      "</div>"
    );
  }
  ...
  function renderResults(result) {
    ...
    const groups = result.findingsByDimension || [];
    const findingsHtml = groups.length
      ? groups.map(dimensionGroupHtml).join("")
      : (result.findings.length
          ? result.findings.map((f, i) => findingCard(f, i)).join("")
          : '<p class="text-sm text-slate-500">No se detectaron patrones de riesgo conocidos. Se recomienda revisión integral por un profesional del derecho.</p>');

    // Banner Cero Tolerancia para Pliegos Tipo
    const ceroToleranciaBanner = result.meta && result.meta.modoEstricto
      ? '<div class="bg-gradient-to-r from-rose-50 to-orange-50 border-2 border-rose-300 rounded-xl p-4 mb-5 flex items-start gap-3 animate-fade-in">' +
        '<svg class="w-6 h-6 text-rose-700 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>' +
        '<div><p class="text-sm font-bold text-rose-900">Modo CERO TOLERANCIA · Pliegos Tipo</p>' +
        '<p class="text-xs text-rose-700 mt-1 leading-relaxed">Este proceso está sujeto a Documentos Tipo (Ley 1882/2018). Cualquier desviación de los requisitos habilitantes del Documento Base es <b>causal de nulidad del proceso</b>. Las severidades de las reglas pertinentes se han elevado automáticamente.</p>' +
        "</div></div>"
      : "";
    ...
    panel.innerHTML =
      ...
      ceroToleranciaBanner +
      capacidadResidualHtml(result) +
      '<div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 animate-fade-in">' +
      '<h3 class="text-base font-semibold text-slate-900 mb-1">Dictamen estratificado por dimensión</h3>' +
      '<p class="text-xs text-slate-500 mb-4">Habilitantes · Ponderables · Causales de Rechazo · Pliegos Tipo · Riesgos · Transparencia</p>' +
      findingsHtml + "</div>" +
      '<div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 animate-fade-in">' +
  ...
  async function runSecopSearch(reset) {
    ...
    const tbl = $("#sec-table"); if (tbl) tbl.classList.add("hidden");
    ...
    try {
      ...
      // Guardamos los datos para vistas alternas y export CSV
      secopState.lastData = renderItems;
      $("#sec-results").innerHTML = renderItems.map(secopCard).join("");
      bindSecopCardActions();
      // Mostrar el toggle Lista/Tabla y aplicar vista activa
      const tog = $("#sec-view-toggle");
      if (tog) { tog.classList.remove("hidden"); tog.classList.add("flex"); }
      if (typeof secopApplyView === "function") secopApplyView();
      else $("#sec-results").classList.remove("hidden");
      // Estado/paginación
  ...
  function initSecopOnce() {
    ...
    // Auto-trigger en cambios de filtros (búsqueda inteligente sin botón)
    ["sec-departamento","sec-modalidad","sec-tipo","sec-estado","sec-orden"].forEach((id) => {
      const el = $("#" + id);
      if (el) el.addEventListener("change", () => {
        if (secopState.initialized && (secopState.filters && Object.keys(secopState.filters).length))
          runSecopSearch(true);
      });
    });
    // Enter en campos de texto de filtros
    ["sec-entidad","sec-ciudad","sec-pmin","sec-pmax","sec-fdesde","sec-fhasta","sec-proveedor"].forEach((id) => {
      const el = $("#" + id);
      if (el) el.addEventListener("keydown", (e) => { if (e.key === "Enter") runSecopSearch(true); });
    });
    // Exportar CSV de los resultados actuales
    const btnCsv = $("#sec-export-csv");
    if (btnCsv) btnCsv.addEventListener("click", secopExportCSV);
    // Vista: Lista vs Tabla oficial
    $$(".sec-vtab").forEach((b) => b.addEventListener("click", () => {
      secopState.view = b.dataset.secView;
      secopApplyView();
    }));
    secopState.view = "lista";
  }

  /* Aplica vista activa (lista de tarjetas / tabla oficial). */
  function secopApplyView() {
    $$(".sec-vtab").forEach((b) =>
      b.classList.toggle("active", b.dataset.secView === secopState.view)
    );
    const list = $("#sec-results");
    const table = $("#sec-table");
    if (secopState.view === "tabla") {
      if (list) list.classList.add("hidden");
      secopRenderTable(secopState.lastData || []);
  ...
  /* Renderiza los resultados SECOP como TABLA estilo oficial. */
  function secopRenderTable(items) {
    const host = $("#sec-table");
    if (!host) return;
    if (!items.length) { host.innerHTML = ""; host.classList.add("hidden"); return; }
    const isProcesos = secopState.dataset === "procesos";
    const rows = items.map((p) => {
      const n = SECOP.normalize(secopState.dataset, p);
      const fecha = (n.fecha || "").slice(0, 10);
      const valor = formatCOP(Number(n.valor) || 0);
      const estado = n.estado || "—";
      const urlDet = n.url || "#";
      return (
        '<tr class="hover:bg-sky-50/40 transition-colors">' +
        '<td class="px-3 py-2.5 text-xs text-slate-800 font-medium max-w-xs truncate" title="' + escapeHtml(n.objeto || "") + '">' + escapeHtml(n.objeto || "—") + "</td>" +
        '<td class="px-3 py-2.5 text-xs text-slate-700">' + escapeHtml(n.entidad || "—") + "</td>" +
        '<td class="px-3 py-2.5 text-xs text-slate-600">' + escapeHtml(n.departamento || "—") + "</td>" +
        '<td class="px-3 py-2.5 text-xs text-slate-600">' + escapeHtml(n.modalidad || "—") + "</td>" +
        '<td class="px-3 py-2.5 text-xs text-slate-800 font-bold text-right">' + valor + "</td>" +
        '<td class="px-3 py-2.5 text-xs text-slate-600">' + escapeHtml(fecha) + "</td>" +
        '<td class="px-3 py-2.5 text-xs text-center"><span class="inline-block bg-sky-100 text-sky-700 text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full">' + escapeHtml(estado) + "</span></td>" +
        '<td class="px-3 py-2.5 text-xs text-center">' +
          (urlDet !== "#" ? '<a href="' + escapeHtml(urlDet) + '" target="_blank" rel="noopener" class="text-sky-700 hover:underline font-semibold">Detalle</a>' : '<span class="text-slate-400">—</span>') +
        "</td>" +
        "</tr>"
      );
    }).join("");
    host.innerHTML =
      ...
      '<th class="px-3 py-2 text-left text-[10px] font-bold uppercase tracking-wider text-slate-600">Objeto</th>' +
      '<th class="px-3 py-2 text-left text-[10px] font-bold uppercase tracking-wider text-slate-600">Entidad</th>' +
      '<th class="px-3 py-2 text-left text-[10px] font-bold uppercase tracking-wider text-slate-600">Departamento</th>' +
      '<th class="px-3 py-2 text-left text-[10px] font-bold uppercase tracking-wider text-slate-600">' + (isProcesos ? "Modalidad" : "Tipo") + "</th>" +
      '<th class="px-3 py-2 text-right text-[10px] font-bold uppercase tracking-wider text-slate-600">' + (isProcesos ? "Presupuesto" : "Cuantía") + "</th>" +
      '<th class="px-3 py-2 text-left text-[10px] font-bold uppercase tracking-wider text-slate-600">Fecha</th>' +
      '<th class="px-3 py-2 text-center text-[10px] font-bold uppercase tracking-wider text-slate-600">Estado</th>' +
      '<th class="px-3 py-2 text-center text-[10px] font-bold uppercase tracking-wider text-slate-600">Acciones</th>' +
      "</tr></thead>" +
      '<tbody class="bg-white divide-y divide-slate-100">' + rows + "</tbody>" +
      "</table></div>";
  }

  /* Exporta los resultados SECOP actuales a CSV (separador ; · UTF-8 BOM). */
  function secopExportCSV() {
    const rows = secopState.lastData || [];
    if (!rows.length) { toast("No hay resultados para exportar. Realiza una búsqueda primero.", "warn"); return; }
    const norm = rows.map((p) => SECOP.normalize(secopState.dataset, p));
    const header = ["Objeto","Entidad","Departamento","Ciudad","Modalidad/Tipo","Estado","Fecha","Valor (COP)","ID","URL"];
    const esc = (v) => {
    ...
    norm.forEach((n) => {
      lines.push([
        n.objeto || "", n.entidad || "", n.departamento || "", n.ciudad || "",
        n.modalidad || n.tipoContrato || "", n.estado || "",
        (n.fecha || "").slice(0, 10), n.valor || 0,
        n.id || "", n.url || "",
      ].map(esc).join(";"));
    });
    ...
    a.href = url; a.download = "SECOP-" + secopState.dataset + "-" + ts + ".csv";
    document.body.appendChild(a); a.click();
    setTimeout(() => { document.body.removeChild(a); URL.revokeObjectURL(url); }, 100);
    toast("CSV descargado · " + norm.length + " registros", "ok");
  }
  ...
  function bindEvents() {
    ...
    // Importador SECOP por URL/ID
    const btnImp = $("#btnImportarProceso");
    if (btnImp) btnImp.addEventListener("click", importarProcesoParaAnalisis);
    const impInput = $("#input-secop-url");
    if (impInput) impInput.addEventListener("keydown", (e) => {
      if (e.key === "Enter") { e.preventDefault(); importarProcesoParaAnalisis(); }
    });
  ...
  function paaCollectFilters() {
    const anio = $("#paa-anio") ? $("#paa-anio").value.trim() : "";
    const modo = $("#paa-modo") ? $("#paa-modo").value : "paa";
    return {
      ...
      precioMax: ($("#paa-pmax") && $("#paa-pmax").value || "").trim(),
      orden: "fecha_estimada_de_inicio ASC",
    };
  }

  /* Limpia todos los filtros del formulario PAA estilo SECOP oficial. */
  function paaClearFilters() {
    ["paa-q", "paa-entidad", "paa-pmin", "paa-pmax"].forEach((id) => {
      const el = $("#" + id); if (el) el.value = "";
    });
    if ($("#paa-anio")) $("#paa-anio").value = String(new Date().getFullYear());
    if ($("#paa-modo")) $("#paa-modo").value = "paa";
    if ($("#paa-departamento")) $("#paa-departamento").value = "";
    if ($("#paa-modalidad")) $("#paa-modalidad").value = "";
    // Estado interno
    paaState.page = 0;
    paaState.lastData = [];
    // Volver al empty-state
    ["paa-results", "paa-analytics", "paa-charts", "paa-view-toggle", "paa-calendar", "paa-pagination", "paa-error", "paa-table"].forEach((id) => {
      const el = $("#" + id); if (el) el.classList.add("hidden");
    });
    const empty = $("#paa-empty"); if (empty) empty.classList.remove("hidden");
  }

  /* Exporta los resultados actuales del PAA a CSV (formato SECOP oficial). */
  function paaExportCSV() {
    const rows = paaState.lastData || [];
    if (!rows.length) { toast("No hay resultados para exportar. Realiza una búsqueda primero.", "warn"); return; }
    const norm = rows.map((p) => SECOP.normalize("paa", p));
    const header = ["Entidad","Año","Fecha de publicación","Valor total del PAA","Versión","Fecha de modificación","Estado","Objeto","Modalidad","Departamento"];
    const esc = (v) => {
      const s = String(v == null ? "" : v).replace(/"/g, '""');
      return /[",\n;]/.test(s) ? '"' + s + '"' : s;
    };
    const lines = [header.join(";")];
    norm.forEach((n) => {
      const anio = n.fecha ? new Date(n.fecha).getFullYear() : "";
      lines.push([
        n.entidad || "",
        anio,
        n.fechaPublicacion || n.fecha || "",
        n.valor || 0,
        n.version || "1",
        n.fechaModificacion || n.fecha || "",
        n.estado || "Publicado",
        n.objeto || "",
        n.modalidad || "",
        n.departamento || "",
    ...
    const a = document.createElement("a");
    const ts = new Date().toISOString().slice(0, 10);
    a.href = url; a.download = "PAA-busqueda-" + ts + ".csv";
    document.body.appendChild(a); a.click();
    setTimeout(() => { document.body.removeChild(a); URL.revokeObjectURL(url); }, 100);
    toast("CSV descargado · " + norm.length + " registros", "ok");
  }

  /* Renderiza los resultados PAA como TABLA estilo SECOP oficial. */
  function paaRenderTable(items) {
    let host = $("#paa-table");
    if (!host) {
      host = document.createElement("div");
      host.id = "paa-table";
      host.className = "hidden bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mb-5";
      const results = $("#paa-results");
      if (results && results.parentNode) results.parentNode.insertBefore(host, results);
    }
    if (!items.length) { host.classList.add("hidden"); return; }
    const rows = items.map((p) => {
      const n = SECOP.normalize("paa", p);
      const anio = n.fecha ? new Date(n.fecha).getFullYear() : "—";
      const fp = n.fechaPublicacion || n.fecha || "";
      const fm = n.fechaModificacion || n.fecha || "";
      const valor = formatCOP(Number(n.valor) || 0);
      const estado = n.estado || "Publicado";
      const ver = n.version || "1";
      const detalleUrl = n.url || "#";
      return (
        "<tr class=\"hover:bg-amber-50/40 transition-colors\">" +
        '<td class="px-3 py-2.5 text-xs text-slate-800 font-medium">' + escapeHtml(n.entidad || "—") + "</td>" +
        '<td class="px-3 py-2.5 text-xs text-slate-600 text-center">' + anio + "</td>" +
        '<td class="px-3 py-2.5 text-xs text-slate-600">' + escapeHtml(String(fp).slice(0, 10)) + "</td>" +
        '<td class="px-3 py-2.5 text-xs text-slate-800 font-bold text-right">' + valor + "</td>" +
        '<td class="px-3 py-2.5 text-xs text-slate-600 text-center">' + ver + "</td>" +
        '<td class="px-3 py-2.5 text-xs text-slate-600">' + escapeHtml(String(fm).slice(0, 10)) + "</td>" +
        '<td class="px-3 py-2.5 text-xs text-center"><span class="inline-block bg-emerald-100 text-emerald-700 text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full">' + escapeHtml(estado) + "</span></td>" +
        '<td class="px-3 py-2.5 text-xs text-center">' +
          (detalleUrl !== "#" ? '<a href="' + escapeHtml(detalleUrl) + '" target="_blank" rel="noopener" class="text-sky-700 hover:underline font-semibold">Detalle</a>' : '<span class="text-slate-400">—</span>') +
        "</td>" +
        "</tr>"
      );
    }).join("");
    host.innerHTML =
      ...
      '<thead class="bg-slate-50"><tr>' +
      '<th class="px-3 py-2 text-left text-[10px] font-bold uppercase tracking-wider text-slate-600">Entidad</th>' +
      '<th class="px-3 py-2 text-center text-[10px] font-bold uppercase tracking-wider text-slate-600">Año</th>' +
      '<th class="px-3 py-2 text-left text-[10px] font-bold uppercase tracking-wider text-slate-600">Fecha publicación</th>' +
      '<th class="px-3 py-2 text-right text-[10px] font-bold uppercase tracking-wider text-slate-600">Valor total del PAA</th>' +
      '<th class="px-3 py-2 text-center text-[10px] font-bold uppercase tracking-wider text-slate-600">Versión</th>' +
      '<th class="px-3 py-2 text-left text-[10px] font-bold uppercase tracking-wider text-slate-600">Fecha modificación</th>' +
      '<th class="px-3 py-2 text-center text-[10px] font-bold uppercase tracking-wider text-slate-600">Estado</th>' +
      '<th class="px-3 py-2 text-center text-[10px] font-bold uppercase tracking-wider text-slate-600">Acciones</th>' +
      "</tr></thead>" +
      '<tbody class="bg-white divide-y divide-slate-100">' + rows + "</tbody>" +
  ...
  function paaApplyView() {
    ...
    const results = $("#paa-results");
    const cal = $("#paa-calendar");
    const tabla = $("#paa-table");
    const pag = $("#paa-pagination");
    [results, cal, tabla].forEach((el) => el && el.classList.add("hidden"));
    if (paaState.view === "lista") {
      results.classList.remove("hidden");
      pag.classList.remove("hidden");
    } else if (paaState.view === "calendario") {
      cal.classList.remove("hidden");
      pag.classList.add("hidden");
      renderPaaCalendar(paaState.lastData);
      bindPaaCardActions();
    } else if (paaState.view === "tabla") {
      paaRenderTable(paaState.lastData);
  ...
  async function initPaaOnce() {
    ...
    // Nuevos controles del formulario estilo SECOP oficial
    const btnAdv = $("#paa-search-adv");
    if (btnAdv) btnAdv.addEventListener("click", () => runPaaSearch(true));
    const btnClear = $("#paa-clear");
    if (btnClear) btnClear.addEventListener("click", paaClearFilters);
    const btnCsv = $("#paa-export-csv");
    if (btnCsv) btnCsv.addEventListener("click", paaExportCSV);
    ["paa-entidad", "paa-pmin", "paa-pmax"].forEach((id) => {
      const el = $("#" + id);
      if (el) el.addEventListener("keydown", (e) => { if (e.key === "Enter") runPaaSearch(true); });
    });
    ["paa-anio", "paa-modo", "paa-departamento", "paa-modalidad"].forEach((id) => {
      const el = $("#" + id);
      if (el) el.addEventListener("change", () => { if (paaState.lastData.length) runPaaSearch(true); });
    });
  ...
  /* =====================================================================
     IMPORTADOR DE PROCESO SECOP · pega URL/ID → analizador
     ===================================================================== */
  function parseSecopId(input) {
    const s = String(input || "").trim();
    if (!s) return null;
    // URL pública con parámetro noticeUID
    const params = s.match(/[?&]noticeUID=([^&\s#]+)/i);
    if (params) return decodeURIComponent(params[1]);
    // Patrón nativo CO1.XXX.YYY
    const co = s.match(/CO1\.[A-Z]{2,5}\.\d+/i);
    if (co) return co[0].toUpperCase();
    // Referencia tipo PN-ESBOL-013-2026 (alfanumérico con guiones)
    if (/^[A-Z0-9]{2,}-/i.test(s) && s.length < 60 && !s.startsWith("http")) return s;
    // Solo números (referencia interna corta)
    if (/^\d{4,}$/.test(s)) return s;
    return null;
  }

  async function importarProcesoParaAnalisis() {
    const inputEl = $("#input-secop-url");
    if (!inputEl) return;
    const raw = inputEl.value.trim();
    const status = $("#importerStatus");
    const btn = $("#btnImportarProceso");

    if (!raw) {
      if (status) {
        status.classList.remove("hidden");
        status.className = "mt-3 text-xs text-rose-300";
        status.textContent = "Pega un link o ID de SECOP.";
      }
      return;
    }
    const id = parseSecopId(raw);
    if (!id) {
      if (status) {
        status.classList.remove("hidden");
        status.className = "mt-3 text-xs text-rose-300";
        status.textContent = "No reconocemos el formato. Esperamos un link de SECOP II o un ID tipo CO1.NTC.123456.";
      }
      return;
    }
    ...
    if (status) {
      status.classList.remove("hidden");
      status.className = "mt-3 text-xs text-sky-200 flex items-center gap-2";
      status.innerHTML =
        '<span class="inline-block w-3 h-3 border-2 border-sky-300 border-t-transparent rounded-full animate-spin"></span>' +
        "Consultando datos abiertos por <code class=\"bg-slate-800 px-1.5 py-0.5 rounded\">" + escapeHtml(id) + "</code>…";
    }
    if (btn) btn.disabled = true;

    try {
      const results = await SECOP.search("procesos", { texto: id, limite: 5 });
      if (!results || !results.length) {
        if (status) {
          status.className = "mt-3 text-xs text-amber-200";
          status.innerHTML = '⚠ No encontramos ese proceso en datos abiertos. Verifica el ID o que el proceso ya esté publicado en SECOP 2.';
        }
        return;
      }
      const n = SECOP.normalize("procesos", results[0]);
      const modMap = {
        "mínima cuantía": "minima_cuantia",
        "selección abreviada": "seleccion_abreviada",
        "licitación pública": "licitacion",
      ...
      if (status) {
        ...
        status.innerHTML = "✓ Proceso encontrado: <b>" + escapeHtml(n.entidad || "Entidad") + "</b> — cargando al analizador…";
      }
      // Prefill del analizador
      $("#f-proceso").value = n.id || id;
      $("#f-entidad").value = n.entidad || "";
      $("#f-objeto").value = n.objeto || "";
      $("#f-modalidad").value = modMap[(n.modalidad || "").toLowerCase()] || "minima_cuantia";
      setTimeout(() => {
        showSection("analisis");
        toast("Proceso " + (n.id || id) + " importado · pega el requisito a analizar", "ok");
        if (status) status.classList.add("hidden");
      }, 700);
    } catch (e) {
      if (status) {
        status.className = "mt-3 text-xs text-rose-300";
        status.textContent = "Error al consultar SECOP: " + (e.message || String(e));
      }
    } finally {
